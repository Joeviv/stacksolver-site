...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Header Móvil */}
      <div className="sticky top-0 z-40 md:hidden flex items-center justify-between px-6 py-4 border-b border-zinc-800 bg-black/80 backdrop-blur-xl">
        <div className="flex items-center gap-2">
             <div className="w-6 h-6 bg-blue-600 rounded-md flex items-center justify-center">
                <CodeBracketIcon className="w-4 h-4 text-white" />
             </div>
            <span className="font-bold text-white">StackSolver</span>
        </div>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 bg-black border-r border-zinc-800
      ...
      `}>
        ...
        <a href="/" className="p-8 flex-shrink-0 cursor-pointer block group">
            <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-lg font-bold tracking-tight text-white">StackSolver</h1>
            </div>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto pb-20 custom-scrollbar">
            <div className="pb-4">
                <a 
                    href="/"
                    className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-lg transition-all duration-200 mb-2 ${isActive('/') && currentPath === '/' ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}
                >
                        ...
                        <div key={section.id} className="mb-1">
                            <button
                                ...
                                className={`
                                    ...
                                    ${isSectionActive ? 'text-white' : 'text-zinc-400 hover:bg-zinc-800 hover:text-zinc-100'}
                                `}
                            >
                                <div className="flex items-center gap-3">
                                    <Icon className={`w-5 h-5 flex-shrink-0 transition-transform ${isSectionActive ? 'text-blue-500' : 'text-zinc-500 group-hover:text-zinc-300'}`} />
                ...
                <p className="px-4 text-xs font-bold text-zinc-600 uppercase tracking-wider mb-3 mt-6">Empresa</p>
                 <a 
                    href="/nosotros"
                    className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-lg transition-all duration-200 ${isActive('/nosotros') ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}
                >
                 ...
                 <a 
                    href="/contacto"
                    className={`w-full flex items-center space-x-3 px-4 py-3.5 rounded-lg transition-all duration-200 ${isActive('/contacto') ? 'bg-zinc-900 text-white' : 'text-zinc-400 hover:bg-zinc-800'}`}
                >
